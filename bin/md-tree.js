...
class MarkdownCLI {
  ...
  async writeFile(filePath, content) {
    try {
      ...
      console.log(`✅ Written to ${path.relative(process.cwd(), resolvedPath)}`);
    } catch (error) {
...
For more information, visit: https://github.com/your-username/markdown-tree-parser
`);
  }
  ...
  async listHeadings(filePath, format = 'text') {
    ...
    if (format === 'json') {
      console.log(JSON.stringify(headings.map(h => ({
        level: h.level,
        text: h.text
      })), null, 2));
    } else {
  ...
  async extractSection(filePath, headingText, outputDir = null) {
    ...
    if (!section) {
      console.error(`❌ Section "${headingText}" not found in ${path.basename(filePath)}`);

      // Suggest similar headings
      const headings = this.parser.getHeadingsList(tree);
      const suggestions = headings
        .filter(h => h.text.toLowerCase().includes(headingText.toLowerCase().substring(0, 3)))
        .slice(0, 3);
  ...
  async extractAllSections(filePath, level = 2, outputDir = null) {
    ...
    if (sections.length === 0) {
      console.log(`⚠️  No sections found at level ${level} in ${path.basename(filePath)}`);
      return;
    }
    ...
    if (format === 'json') {
      ...
      nodes.forEach((node, index) => {
        ...
        if (node.type === 'heading') {
          ...
          console.log(`   Value: "${preview}${node.value.length > 100 ? '...' : ''}"`);
  ...
  async generateTOC(filePath, maxLevel = 3) {
    ...
    if (!toc) {
      console.log(`⚠️  No headings found in ${path.basename(filePath)} to generate TOC`);
      return;
    }
